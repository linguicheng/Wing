...
namespace Wing.APM.SqlSugar
{
    public class SqlSugarDiagnosticListener : IDiagnosticListener
    {
        ...
        {
            var data = ListenerTracer.GetProperty<OnLogExecutingModel>(value, "Value");
            var id = data.ContextID.ToString();
            TracerDto tracerDto;
        ...
        {
            var data = ListenerTracer.GetProperty<OnLogExecutedModel>(value, "Value");
            var id = data.ContextID.ToString();
            TracerDto tracerDto;
            var context = _httpContextAccessor.HttpContext;
            if (context == null)
        ...
        {
            var data = ListenerTracer.GetProperty<OnErrorModel>(value, "Value");
            var id = data.ContextID.ToString();
            TracerDto tracerDto;
            var context = _httpContextAccessor.HttpContext;
            if (context == null)
        ...
        private string GetAction(SugarActionType actionType)
        {
            switch (actionType)
            {
                ...
                case SugarActionType.UnKnown:
                    return ApmTools.Sql_Action_UnKnown;
                default:
                    return string.Empty;
            }
        }
