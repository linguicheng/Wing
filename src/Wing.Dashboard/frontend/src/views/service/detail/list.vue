     ...
     <div class="table-container">
      <vxe-table ref="xTable"
        ...
        <vxe-column
          ...
          fixed="right">
            <template #default="{ row }">
              <el-button type="success" @click.prevent="toConfig(row)" style="margin-right:5px;">配置</el-button>
              <el-popconfirm
                  title="确定要删除吗？"
  ...
  methods: {
    ...
    toConfig (row) {
      ...
      const timer = setTimeout(() => {
        this.Bus.emit('serviceNameToConfig', row.configKey)
        clearTimeout(timer)
      })
    },
    async remove (row) {
